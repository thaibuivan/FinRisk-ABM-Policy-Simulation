"""Generate v2 synthetic transaction-risk simulation data.
...
def make_amount(rng: np.random.Generator, median: float, is_fraud: bool, strategy: str | None) -> float:
    ...
    if strategy == "low_amount_probe":
        amount = rng.lognormal(mean=np.log(max(median * 0.9, 1.0)), sigma=0.55)
    elif strategy == "burst_velocity":
        amount = rng.lognormal(mean=np.log(max(median * 2.8, 2.0)), sigma=0.95)
    elif strategy == "merchant_abuse":
        amount = rng.lognormal(mean=np.log(max(median * 5.5, 5.0)), sigma=1.05)
    else:
        amount = rng.lognormal(mean=np.log(max(median * 10.0, 10.0)), sigma=1.10)
...
def make_transactions(config: SimulationConfig, customers: pd.DataFrame, rng: np.random.Generator) -> pd.DataFrame:
    ...
    strategy_probs = [0.30, 0.06, 0.30, 0.16, 0.15, 0.03]
    rows = []
    tx_no = 1
    ...
    for day in range(config.days):
        ...
        for customer in customers.itertuples(index=False):
            ...
            for _ in range(tx_count):
                baseline_multiplier = 0.35 + 8.0 * float(customer.baseline_risk)
                fraud_probability = min(config.fraud_rate * baseline_multiplier, config.fraud_rate * 4.0)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate v2 synthetic transaction-risk simulation data.")
