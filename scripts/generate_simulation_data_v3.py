"""Generate v3 BankSim/PaySim-calibrated synthetic transaction-risk simulation data.
...
It does not copy rows from public datasets. Version 3 uses stronger calibration
ideas from BankSim and PaySim profiles: fraud remains rare, but it is more
strongly associated with amount anomaly, high-risk merchant/category,
late-hour behavior and customer baseline risk. This makes the generated data
more suitable for evaluating PR-AUC and recall.
"""
...
def make_amount(rng: np.random.Generator, median: float, is_fraud: bool, strategy: str | None) -> float:
    if not is_fraud:
        ...
        if rng.random() < 0.004:
            amount *= rng.uniform(1.8, 3.5)
        return round(float(min(max(amount, 1.0), 5000.0)), 2)

    if strategy == "low_amount_probe":
        amount = rng.lognormal(mean=np.log(max(median * 1.25, 1.0)), sigma=0.45)
    elif strategy == "burst_velocity":
        amount = rng.lognormal(mean=np.log(max(median * 4.2, 2.0)), sigma=0.85)
    elif strategy == "merchant_abuse":
        amount = rng.lognormal(mean=np.log(max(median * 8.5, 5.0)), sigma=0.95)
    else:
        amount = rng.lognormal(mean=np.log(max(median * 15.0, 10.0)), sigma=0.95)
...
def make_transactions(config: SimulationConfig, customers: pd.DataFrame, rng: np.random.Generator) -> pd.DataFrame:
    ...
    strategy_probs = [0.35, 0.025, 0.31, 0.15, 0.14, 0.025]
    rows = []
    tx_no = 1
    ...
    for day in range(config.days):
        ...
        for customer in customers.itertuples(index=False):
            ...
            for _ in range(tx_count):
                baseline_multiplier = 0.25 + 10.0 * float(customer.baseline_risk)
                fraud_probability = min(config.fraud_rate * baseline_multiplier, config.fraud_rate * 5.0)
                is_fraud = bool(rng.random() < fraud_probability)
                strategy = None
                if is_fraud:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate v3 BankSim/PaySim-calibrated synthetic transaction-risk simulation data.")
