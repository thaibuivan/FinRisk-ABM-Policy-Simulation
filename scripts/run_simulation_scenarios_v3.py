"""Run v3 BankSim/PaySim-calibrated transaction-risk simulation scenarios and compare KPIs.
...
def run_one_scenario(project_root: Path, scenario: Scenario, output_dir: Path) -> Path:
    generator = project_root / "scripts" / "generate_simulation_data_v3.py"
    cmd = [
        sys.executable,
        str(generator),
        "--customers",
...
def write_markdown_summary(comparison: pd.DataFrame, output_path: Path) -> None:
    ...
    markdown = [
        "# Scenario Comparison Summary - v3 BankSim/PaySim-calibrated simulation",
        "",
